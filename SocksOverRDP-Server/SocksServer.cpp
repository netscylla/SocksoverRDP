...
int CheckAuthentication(char *buf_full, int ret)
{
	...
	char	*buf, answer[2 + sizeof(DWORD) + sizeof(DWORD) + 1];
	struct threads *pta;
...
{
	char	null[20],  answer[300 + sizeof(DWORD) + sizeof(DWORD) + 1];
	//int *answer;
	DWORD   dwWritten, ret;
...
int getAddressInfo(sockaddr_in *sockaddrin, sockaddr_in6 *sockaddrin6, char *buf, int ret)
{
	...
	if (buf[0] == 5)
	{
		...
		if (buf[3] == 3)
		{
			...
			SecureZeroMemory(&hints, 48);
			SecureZeroMemory(domain, 256);
...
DWORD WINAPI HandleClient(void *param)
{
	...
	DWORD		dwRecvN, dwRet, dwSentN, dwSentPointer, Flags;
